...
// ---------------------------
// 5) EVENT INDICATOR ATTRIBUTES & HIGHLIGHT
// ---------------------------
const indicatorAttributes = computed(() => {
  const counts = props.eventCounts
  if (!counts) return []

  const attrs: any[] = []
  const indicatorColor = { style: { backgroundColor: '#2A9749' } }

  for (const [iso, count] of Object.entries(counts)) {
    if (!count) continue
    const date = isoToDate(iso)
    const indicators = Math.min(count, props.indicatorStyle === 'bar' ? 3 : 4)

    for (let i = 0; i < indicators; i++) {
      attrs.push({
        key: `${iso}-${i}`,
        dates: date,
        ...(props.indicatorStyle === 'bar'
          ? { bar: indicatorColor }
          : { dot: indicatorColor }),
        order: 5,
      })
